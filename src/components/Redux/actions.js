...
export function getProductsByName(nombre) {
  ...
  return function (dispatch) {
    ...
    axios.get(`http://localhost:3001/productos?name=${nombre}`, { headers: { "Authorization": `Bearer ${token}` } })
      .then((r) => {
        console.log(r.data);
        dispatch({
...
export const usuario_autenticado = async () => {
  const token = localStorage.getItem("token");
  axios.get("http://localhost:3001/productos?name=sal", { headers: { "Authorization": `Bearer ${token}` } })
    .then(res => {
...
export const usuarioAutenticado = async () => {
  ...
  try {
    const respuesta = await axios.get(
      "http://localhost:3001/productos?name=sal"
    );
    console.log(respuesta);
    return function (dispatch) {
      dispatch({
